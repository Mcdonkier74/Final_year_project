    ...
    def _apply_policy(self, state, action, reward):
        ...
        if self.old_state:
            ...
            if len(l_aux) > 0:
                max_Q = max(l_aux)
            # update qtable
            gamma_f_max_Q_a_prime = .01 * max_Q
...
class Policy_Gradient(BasicLearningAgent):
	...
	def _choose_an_action(self, t_state, valid_actions):
		...
		s_print += s_aux.format(self.f_gamma, self.gamma)
...
def run(s_option):
    ...
    """
    for i in range(0,15):
        i_idx = 10   # 15  # index of the start file to be used in simulations
        n_trials = 1  # number of repetitions of the same sessions
