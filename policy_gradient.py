...
class Policy_Gradient(BasicLearningAgent):
    """
    The policy gradient algorithm that learns to trade given a set learning rate
    """

    def __init__(self, env, i_id, f_sname=None, f_min_time=3600, f_gamma=0.99):
        """
        initialise our policy gradient.... from BasicLearningAgent
        we will be using keras backend
        """
	self.env = env
	self.i_id = i_id
        self.f_sname = f_sname
	self.f_min_time = f_min_time
	self.f_gamma = f_gamma
	
	from keras.optimizers import SGD
        self.model = MarketPolicyGradientModelBuilder(f_sname).getModel()
        sgd = SGD(lr = 0.1, decay = 1e-6, momentum = 0.9, nesterov = True)
        self.model.compile(loss='mse', optimizer='rmsprop')

    def _choose_an_action(self, r):
	...
	while not game_over:
		...
		y[action] = 1.
            ...
            if verbose > 0:
               if env.actions[action] == "ASK" or env.actions[action] == "BID":
...
def run(s_option):
    ...
    modelFilename = 'data/data_0725_0926.zip'
	...
	a = e.create_agent(Policy_Gradient,
                           f_min_time=2.,
                           #f_k=0.8,
                           f_gamma=0.5)
